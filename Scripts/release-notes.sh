...
#
# The release workflow pipes the Markdown form into `gh release create --notes-file`, and feeds
# `--plain` into App Store Connect's "What's New". One source for both, so the changelog, the
# GitHub release and the store cannot drift into describing the same build differently.
#
