#!/bin/bash
#
# Packages the notarized download build into a disk image for the GitHub release.
#
#   Scripts/make-dmg.sh <path/to/SubTrack.app> <version> <output-directory>
#
# Produces "<output-directory>/SubTrack-<version>.dmg" and prints its path. The image holds the
...
mkdir -p "${OUT_DIR}"
DMG="${OUT_DIR}/SubTrack-${VERSION}.dmg"
rm -f "${DMG}"
